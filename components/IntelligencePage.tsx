...
function Card({ title, body, points }: { title: string; body: string; points?: string[] }) {
  return (
    <div className="card p-6">
      ...
      {body ? <p className="mt-4 text-sm leading-7 text-ivory/55">{body}</p> : null}
      {points ? (
        <ul className="mt-5 space-y-2 text-sm text-ivory/60">
          {points.map((point) => (
      ...
      <Section eyebrow="Territory Coverage" title="Korea-to-Global and Global-to-Korea opportunity categories.">
        <div className="grid gap-3 md:grid-cols-3">
          {intelligenceCategories.map((item) => (
            <div key={item} className="rounded border border-white/10 bg-white/[0.03] px-4 py-3 text-sm font-semibold text-ivory/75">{item}</div>
          ))}
        </div>
      </Section>
      ...
      <Section eyebrow={copy.trust.eyebrow} title={copy.trust.title}>
        <div className="grid gap-5 md:grid-cols-3">
          {copy.trust.notes.map((note) => (
            <div key={note} className="rounded border border-champagne/25 bg-champagne/[0.06] p-6 text-sm leading-7 text-ivory/70">{note}</div>
          ))}
        </div>
      </Section>
    </>
