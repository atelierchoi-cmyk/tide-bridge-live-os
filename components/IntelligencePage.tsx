...
function Card({ title, body, points }: { title: string; body: string; points?: string[] }) {
  return (
    <div className="card p-6">
      ...
      {body ? <p className="cjk-copy mt-4 whitespace-pre-line break-keep text-sm leading-7 text-ivory/55">{body}</p> : null}
      {points ? (
        <ul className="mt-5 space-y-2 break-keep text-sm text-ivory/60">
          {points.map((point) => (
            <li key={point} className="flex gap-2">
              ...
              <span className="cjk-copy whitespace-pre-line">{point}</span>
            </li>
          ))}
        </ul>
      ) : null}
    ...
    <>
      <section className="mx-auto max-w-7xl px-5 py-24 md:py-32">
        <div className="max-w-4xl">
          ...
          <p className="cjk-copy mt-4 max-w-3xl whitespace-pre-line break-keep text-base leading-8 text-ivory/55">{copy.hero.support}</p>
      ...
      <Section eyebrow="Territory Coverage" title="Korea-to-Global and Global-to-Korea opportunity categories.">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {intelligenceCategories.map((item) => (
            <div key={item} className="cjk-copy break-keep rounded border border-white/10 bg-white/[0.03] px-4 py-3 text-sm font-semibold text-ivory/75">{item}</div>
          ))}
        </div>
      </Section>
      ...
      <Section eyebrow={copy.trust.eyebrow} title={copy.trust.title}>
        <div className="grid gap-5 lg:grid-cols-3">
          {copy.trust.notes.map((note) => (
            <div key={note} className="cjk-copy whitespace-pre-line break-keep rounded border border-champagne/25 bg-champagne/[0.06] p-6 text-sm leading-7 text-ivory/70">{note}</div>
          ))}
        </div>
      </Section>
    </>
