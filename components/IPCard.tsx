...
export function IPCard({ item, privateMode = false }: { item: IPPackage; privateMode?: boolean }) {
  const href = privateMode ? `/partner/ip/${item.id}` : '/apply';
  return (
    <div className="card flex h-full flex-col p-6">
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 text-sm">
        <div className="rounded-2xl bg-white/[0.04] p-4">
          <p className="text-ivory/40">Capacity</p>
          <p className="mt-1 font-semibold text-ivory">{item.capacityRange}</p>
        </div>
        <div className="rounded-2xl bg-white/[0.04] p-4">
          <p className="text-ivory/40">Base</p>
      ...
      <Link href={href} className="btn-secondary mt-7 text-center">{privateMode ? 'View Details' : 'Request Access'}</Link>
    </div>
  );
}
